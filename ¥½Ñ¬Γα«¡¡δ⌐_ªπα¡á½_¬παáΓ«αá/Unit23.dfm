object Form23: TForm23
  Left = 256
  Top = 105
  BorderStyle = bsSingle
  Caption = #1040#1082#1090#1080#1074' '#1091#1095#1077#1073#1085#1086#1081' '#1075#1088#1091#1087#1087#1099
  ClientHeight = 586
  ClientWidth = 866
  Color = clSkyBlue
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 8
    Top = 8
    Width = 849
    Height = 569
    TabOrder = 0
    object Label1: TLabel
      Left = 288
      Top = 16
      Width = 188
      Height = 20
      Caption = #1040#1082#1090#1080#1074' '#1091#1095#1077#1073#1085#1086#1081' '#1075#1088#1091#1087#1087#1099
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 16
      Top = 48
      Width = 91
      Height = 16
      Caption = '1. '#1057#1090#1072#1088#1086#1089#1090#1072':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 16
      Top = 80
      Width = 191
      Height = 16
      Caption = '2. '#1047#1072#1084#1077#1089#1090#1080#1090#1077#1083#1100' '#1089#1090#1072#1088#1086#1089#1090#1099':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 16
      Top = 112
      Width = 480
      Height = 16
      Caption = '3. '#1057#1077#1082#1088#1077#1090#1072#1088#1100' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1080' '#1086#1073#1097#1077#1089#1090#1074#1077#1085#1085#1086#1075#1086' '#1086#1073#1098#1077#1076#1080#1085#1077#1085#1080#1103' "'#1041#1056#1057#1052'":'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label5: TLabel
      Left = 16
      Top = 144
      Width = 227
      Height = 16
      Caption = '4. '#1055#1088#1086#1092#1089#1086#1102#1079#1085#1099#1081' '#1086#1088#1075#1072#1085#1080#1079#1072#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 16
      Top = 224
      Width = 141
      Height = 16
      Caption = '5. '#1059#1095#1077#1073#1085#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label7: TLabel
      Left = 16
      Top = 256
      Width = 209
      Height = 16
      Caption = '6. '#1048#1085#1092#1086#1088#1084#1072#1094#1080#1086#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 16
      Top = 288
      Width = 278
      Height = 16
      Caption = '7. '#1060#1080#1079#1082#1091#1083#1100#1090#1091#1088#1085#1086'-'#1089#1087#1086#1088#1090#1080#1074#1085#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label9: TLabel
      Left = 16
      Top = 320
      Width = 221
      Height = 16
      Caption = '8. '#1055#1088#1086#1080#1079#1074#1086#1076#1089#1090#1074#1077#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label10: TLabel
      Left = 16
      Top = 352
      Width = 232
      Height = 16
      Caption = '9. '#1050#1091#1083#1100#1090#1091#1088#1085#1086'-'#1084#1072#1089#1089#1086#1074#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label11: TLabel
      Left = 16
      Top = 384
      Width = 266
      Height = 16
      Caption = '10. '#1042#1086#1077#1085#1085#1086'-'#1087#1072#1090#1088#1080#1086#1090#1080#1095#1077#1089#1082#1080#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label12: TLabel
      Left = 16
      Top = 416
      Width = 233
      Height = 16
      Caption = '11. '#1057#1072#1085#1080#1090#1072#1088#1085#1086'-'#1073#1099#1090#1086#1074#1086#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label13: TLabel
      Left = 16
      Top = 448
      Width = 427
      Height = 16
      Caption = '12. '#1057#1077#1082#1090#1086#1088' '#1086#1090#1074#1077#1090#1089#1090#1074#1077#1085#1085#1099#1093' '#1079#1072' '#1073#1077#1079#1086#1087#1072#1089#1085#1099#1077' '#1091#1089#1083#1086#1074#1080#1103' '#1090#1088#1091#1076#1072':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label14: TLabel
      Left = 16
      Top = 480
      Width = 193
      Height = 16
      Caption = '13. '#1056#1077#1076#1072#1082#1094#1080#1086#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label15: TLabel
      Left = 288
      Top = 184
      Width = 167
      Height = 16
      Caption = #1057#1086#1089#1090#1072#1074' '#1089#1086#1074#1077#1090#1072' '#1075#1088#1091#1087#1087#1099
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object GroupBox1: TGroupBox
      Left = 8
      Top = 512
      Width = 833
      Height = 49
      Caption = #1056#1077#1076#1072#1082#1090#1080#1088#1086#1074#1072#1090#1100
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object DBNavigator1: TDBNavigator
        Left = 8
        Top = 16
        Width = 224
        Height = 25
        DataSource = DataSource1
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
        TabOrder = 0
      end
      object BitBtn1: TBitBtn
        Left = 728
        Top = 16
        Width = 99
        Height = 25
        Caption = #1047#1072#1082#1088#1099#1090#1100
        TabOrder = 1
        OnClick = BitBtn1Click
        Kind = bkClose
      end
      object BitBtn2: TBitBtn
        Left = 624
        Top = 16
        Width = 99
        Height = 25
        Caption = #1059#1076#1072#1083#1080#1090#1100
        TabOrder = 2
        OnClick = BitBtn2Click
        Kind = bkAbort
      end
      object BitBtn3: TBitBtn
        Left = 520
        Top = 16
        Width = 99
        Height = 25
        Caption = #1044#1086#1073#1072#1074#1080#1090#1100
        TabOrder = 3
        OnClick = BitBtn3Click
        Kind = bkAll
      end
      object BitBtn4: TBitBtn
        Left = 416
        Top = 16
        Width = 99
        Height = 25
        Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
        TabOrder = 4
        OnClick = BitBtn4Click
        Kind = bkOK
      end
    end
    object DBEdit1: TDBEdit
      Left = 112
      Top = 48
      Width = 209
      Height = 21
      DataField = #1057#1090#1072#1088#1086#1089#1090#1072
      DataSource = DataSource1
      TabOrder = 1
    end
    object DBEdit2: TDBEdit
      Left = 208
      Top = 80
      Width = 209
      Height = 21
      DataField = #1047#1072#1084#1077#1089#1090#1080#1090#1077#1083#1100' '#1089#1090#1072#1088#1086#1089#1090#1099
      DataSource = DataSource1
      TabOrder = 2
    end
    object DBEdit3: TDBEdit
      Left = 496
      Top = 112
      Width = 209
      Height = 21
      DataField = #1057#1077#1082#1088#1077#1090#1072#1088#1100' '#1086#1088#1075#1072#1085#1080#1079#1072#1094#1080#1080' '#1086#1073#1097#1077#1089#1090#1074#1077#1085#1085#1086#1075#1086' '#1086#1073#1098#1077#1076#1080#1085#1077#1085#1080#1103' '#1041#1056#1057#1052
      DataSource = DataSource1
      TabOrder = 3
    end
    object DBEdit4: TDBEdit
      Left = 248
      Top = 144
      Width = 209
      Height = 21
      DataField = #1055#1088#1086#1092#1089#1086#1102#1079#1085#1099#1081' '#1086#1088#1075#1072#1085#1080#1079#1072#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 4
    end
    object DBEdit5: TDBEdit
      Left = 160
      Top = 224
      Width = 209
      Height = 21
      DataField = #1059#1095#1077#1073#1085#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 5
    end
    object DBEdit6: TDBEdit
      Left = 232
      Top = 256
      Width = 209
      Height = 21
      DataField = #1048#1085#1092#1086#1088#1084#1072#1094#1080#1086#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 6
    end
    object DBEdit7: TDBEdit
      Left = 296
      Top = 288
      Width = 209
      Height = 21
      DataField = #1060#1080#1079#1082#1091#1083#1100#1090#1091#1088#1085#1086'-'#1089#1087#1086#1088#1090#1080#1074#1085#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 7
    end
    object DBEdit8: TDBEdit
      Left = 240
      Top = 320
      Width = 209
      Height = 21
      DataField = #1055#1088#1086#1080#1079#1074#1086#1076#1089#1090#1074#1077#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 8
    end
    object DBEdit9: TDBEdit
      Left = 256
      Top = 352
      Width = 209
      Height = 21
      DataField = #1050#1091#1083#1100#1090#1091#1088#1085#1086'-'#1084#1072#1089#1089#1086#1074#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 9
    end
    object DBEdit10: TDBEdit
      Left = 288
      Top = 384
      Width = 209
      Height = 21
      DataField = #1042#1086#1077#1085#1085#1086'-'#1087#1072#1090#1088#1080#1086#1090#1080#1095#1077#1089#1082#1080#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 10
    end
    object DBEdit11: TDBEdit
      Left = 256
      Top = 416
      Width = 209
      Height = 21
      DataField = #1057#1072#1085#1080#1090#1072#1088#1085#1086'-'#1073#1099#1090#1086#1074#1086#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 11
    end
    object DBEdit12: TDBEdit
      Left = 448
      Top = 448
      Width = 209
      Height = 21
      DataField = #1057#1077#1082#1090#1086#1088' '#1086#1090#1074#1077#1090#1089#1090#1074#1077#1085#1085#1099#1093' '#1079#1072' '#1073#1077#1079#1086#1087#1072#1089#1085#1099#1077' '#1091#1089#1083#1086#1074#1080#1103' '#1090#1088#1091#1076#1072
      DataSource = DataSource1
      TabOrder = 12
    end
    object DBEdit13: TDBEdit
      Left = 216
      Top = 480
      Width = 209
      Height = 21
      DataField = #1056#1077#1076#1072#1082#1094#1080#1086#1085#1085#1099#1081' '#1089#1077#1082#1090#1086#1088
      DataSource = DataSource1
      TabOrder = 13
    end
    object Button1: TButton
      Left = 792
      Top = 8
      Width = 43
      Height = 25
      Caption = #1055#1077#1095#1072#1090#1100
      TabOrder = 14
      OnClick = Button1Click
    end
  end
  object ADOConnection1: TADOConnection
    ConnectionString = 
      'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=c:\tmp\home\delphi\' +
      #1069#1083#1077#1082#1090#1088#1086#1085#1085#1099#1081'_'#1078#1091#1088#1085#1072#1083'_'#1082#1091#1088#1072#1090#1086#1088#1072'\BD.mdb;Persist Security Info=False;'
    LoginPrompt = False
    Mode = cmShareDenyNone
    Provider = 'Microsoft.Jet.OLEDB.4.0'
    Left = 600
    Top = 24
  end
  object ADOTable1: TADOTable
    Connection = ADOConnection1
    CursorType = ctStatic
    TableName = #1040#1082#1090#1080#1074' '#1091#1095#1077#1073#1085#1086#1081' '#1075#1088#1091#1087#1087#1099
    Left = 632
    Top = 24
  end
  object DataSource1: TDataSource
    DataSet = ADOTable1
    Left = 664
    Top = 24
  end
end
